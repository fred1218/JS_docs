...
(function (w, d, $, ec, cmutils) {
    ...
    function drawAxis(setting, data, returnCall) {
        var _arr = [], returnObj = null, newopt = {}, _mc = this.mychart, opt = this.option;
        var axisSetting = {
            options: {
                backgroundColor: "#ebeaef",//图标背景，默认是无色透明
                //color: ['#000', 'orange'],//item所采用的颜色组
                ...
                toolbox: {
                    ...
                    feature: {
                        //dataZoom : {show: true},
                        'mark': {'show': false},//辅助标线
                        //'dataView': {'show': true, 'readOnly': true},
                        'magicType': {'show': true, 'type': ['line', 'bar']},//整体转换成不同的显示方式,相关联的可以转,直角系，图
                        //'restore': {'show': true},
                        'saveAsImage': {'show': true}
                    }//功能配置
                },
                grid: {'y': 80, 'y2': 100},//直角坐标系内绘图网格,决定图标占多大空间
                dataZoom: {
                    //backgroundColor:"blue",
                    //dataBackgroundColor:"red",
                    //fillerColor:"yellow",
                ...
                xAxis: {
                    ...
                    'axisLabel': {'interval': 'auto'},//0是全部显示，auto是自动隐藏看不到的
                    //axisLine: {onZero: false},
                    splitLine: {show: true},//显示图标网格
                },//x,图类没有轴显示
                yAxis: [{
                    'type': 'value',
                    //'name': 'GDP（亿元）',//轴名称
        ...
        if (returnCall && returnCall(data)) {
            returnObj = returnCall(data);
        } else {
            returnObj = data;
            $.each(returnObj.dataStore[0].itemData, function (i, v) {
                var _obj = {
                    ...
                    yAxisIndex: 0,
                    smooth: true,
                    //itemStyle: {normal: {areaStyle: {type: 'default'}}},//线性
                    //itemStyle: {
                    //    normal: {
        ...
        if (this.hasTimeLine) {
            ...
            newopt = {
                options: {
                    ...
                    legend: {data: returnObj.dataStore[0].key},
                    xAxis: {data: returnObj.dataStore[0].itemKey},
                    series: _arr
                }
            };
    ...
    function drawPie(setting, data, returnCall) {
        ...
        if (returnCall && returnCall(data)) {
            ...
            _arr.push({
                name: '访问来源',
                type: 'pie',
                radius: ['50%', '70%'],//可以直接写string，表示半径，也可以用数组，之差表示半径['50%', '70%']
                //selectedMode: 'single',//single,multiple
                center: ['50%', '60%'],//圆心所在的位置,left,top
                itemStyle: {
                    normal: {
                        label: {
                            //position: 'inner'
    ...
    cmutils.CMCharts.prototype.drawChart = function (setting, returnCall) {
        var that = this, opt = that.option, _mc = that.mychart, rt = null;
        if (_mc) {
